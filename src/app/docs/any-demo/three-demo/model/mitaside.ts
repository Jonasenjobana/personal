...
import { MisideGLTF } from '../resource-util/resource-list';
/**
 * 米塔
 */
export class MitaSideModel {
  ...
  animeState: any; // 动画状态
  gltfOrgin: GLTF; // 原生
  headCamera: PerspectiveCamera; // 头部相机
  constructor() {
    this.headCamera = new PerspectiveCamera();
  }
  initResource: Promise<GLTF>;
  onLoad(cb?: (res: MitaSideModel) => void): Promise<MitaSideModel> {
    return new Promise(async resolve => {
      MisideGLTF.sourcePromise.then(res => {
